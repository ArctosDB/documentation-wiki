...
### -PartCount
This is the total number of parts in a collection regardless of what the part is as long as it is ```in collection``` or otherwise still part of the collection, thus excludes parts ```used up```, ```deaccessioned``` or otherwise no longer available to a collection. 
