...
The Arctos Reporter will directly produce PDF, which may then be printed. The layout of this method is generally more predictable than generating PDF via the browser, but the server-side tools are limited to strict version of various languages, most notably CSS2.1.

### Bartender

The label generating software which accompanies many thermal transfer printers seems to work very well for feeding those printers. (This is all second-hand, someone please help!)

### Crystal Reports

Various high-end reporting tools are commercially available. These generally accept CSV, which is readily generated by the Arctos Reporter. The costs of acquisition, infrastructure, and development are often prohibitive, but with proper use these powerful tools will produce reports of any type.
